...
func TestCipher_Server_Setup(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"success": { // TODO: remove this test on release with a new one
			/*
				send_hmac_key: 223, 162, 25, 212, 110, 139, 238, 129, 79, 230, 89, 111, 155, 8, 46, 208, 135, 236, 58, 9, 222, 0, 87, 25, 150, 57, 78, 167, 215, 182, 199, 29
			...
			*/
			prepare: func(t *testing.T, c *Cipher) {
				t.Helper()
				require.NoError(t, c.AddSenderInitMessage([]byte{8, 2, 18, 131, 1, 8, 1, 18, 32, 252, 161, 0, 84, 99, 43, 27, 77, 180, 100, 22, 28, 198, 252, 240, 156, 130, 15, 167, 128, 166, 43, 47, 96, 128, 120, 152, 223, 130, 235, 245, 195, 26, 68, 8, 100, 18, 64, 102, 248, 202, 190, 149, 126, 95, 187, 26, 233, 109, 246, 155, 98, 7, 154, 221, 252, 63, 148, 63, 221, 68, 28, 162, 71, 18, 23, 51, 60, 234, 43, 168, 197, 33, 219, 98, 28, 63, 57, 89, 59, 176, 253, 117, 230, 64, 214, 170, 50, 130, 8, 170, 42, 83, 20, 191, 39, 99, 87, 191, 111, 214, 37, 34, 23, 65, 69, 83, 95, 50, 53, 54, 95, 67, 66, 67, 45, 72, 77, 65, 67, 95, 83, 72, 65, 50, 53, 54}))
				...
				require.NoError(t, c.AddSenderPublicKey(&pbSecureMessage.EcP256PublicKey{
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expected.clientHMACKey, c.senderHMACKey)
